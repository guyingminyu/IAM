...
class RunTaskThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            for tc in tclist:
                try:
                    log_info = "=" * 20 + " thread=%s,task=%s,case=%s " % (
                    self.getName(), self.task_id, tc['case_name']) + "=" * 20
    ...
    def execute_api(self, url, heads, params, method='GET', cookies=None, files=None):
        ...
        if method.upper() == 'POST':
            r = requests.request('post', url=url, headers=heads, data=params, cookies=cookies, files=files, timeout=30,
                                 verify=False)
        ...
        for case_id in task_cases:
            ...
            for case_api in case_apis:
                api_info = Api.objects.filter(id=case_api['api_id']).values('api_name', 'api_path', 'api_protocol',
                                                                            'api_method').first()
                pre_steps = Step.objects.filter(CaseApi_id=case_api['id'], step_type='SQL').order_by(
                    'step_sort').values()
                post_steps = Step.objects.filter(CaseApi_id=case_api['id'], step_type='REGEXP').order_by(
                    'step_sort').values()
                assertion = Step.objects.filter(CaseApi_id=case_api['id'], step_type='Assertion').values().first()
                data = {}
                case_api_info = api_info
                case_api_headers = {}
                for cah in eval(case_api.get('api_headers')):
                    case_api_headers[cah.get('header_key')] = cah.get('header_value')
                case_api_info['api_headers'] = case_api_headers
